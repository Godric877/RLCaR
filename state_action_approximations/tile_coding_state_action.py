...
class StateActionFeatureVectorWithTile():
    ...
    def __init__(self,
        ...
        self.num_tiles_per_tiling = np.prod(self.tiles_per_dim)
        self.feature_dims = self.num_actions * self.num_tilings*self.num_tiles_per_tiling
        self.feature_array = np.zeros(shape=(2 + self.tiling_dimensions))
        self.feature_array[0] = self.num_actions
        self.feature_array[1] = self.num_tilings
        self.feature_array[2:] = self.tiles_per_dim
    ...
    def __call__(self, s, done, a) -> np.array:
        ...
        for i in range(len(active_states)):
            feature[a][tuple(active_states[i])] = 1
        return feature.flatten()